...
| xAI Cursor | Code / Chat | Desktop, Extension | Local JSONL agent transcripts without persisted token usage | `~/.cursor/projects/<project>/agent-transcripts/<session>/<session>.jsonl` on Windows, macOS, and Linux; `CURSOR_ROOT` can override the projects root | **Supported** — token values are estimated |
| Cognition Devin | Code / Chat | CLI, Desktop | Local SQLite sessions database with message metadata and usage metrics | `%APPDATA%\\Devin\\cli\\sessions.db` on Windows, or `~/.config/devin/cli/sessions.db`; `COGNITION_DEVIN_DB` can override the database path | **Supported** — persisted usage is used when present and missing fields are estimated |
