...
TOKEN_ACCOUNTING_NOTES = {
    "copilot": (
        "GitHub Copilot",
        "Session totals come from shutdown usage, while output tokens are summed from assistant messages. Input and cache values are not normally attached to each turn; in multi-turn sessions they are allocated using each turn's output-token share, so those per-turn values are estimates while the session total remains exact.",
    ),
    "codex": (
        "OpenAI Codex",
        "Token counts come from token_count event records for each completed turn. The viewer aggregates those exact records and does not infer missing values or copy one invocation's usage onto its tools.",
    ),
    "claude": (
        "Anthropic Claude Code",
        "Token counts come from model-response usage fields, including input, cache, output, and reasoning values when provided. Missing values remain unavailable; tool calls do not receive separate token counts.",
    ),
    "antigravity": (
        "Google Antigravity",
    ...
    "m365_copilot": (
        ...
        "Token values are read from exported usage metadata when present and aggregated from the parsed turns. Missing values are left unavailable rather than fabricated.",
    ),
}


def token_accounting_note(provider: str, turn_count: int) -> str:
    """Render the provider-specific explanation shown above session usage."""
    provider_name, description = TOKEN_ACCOUNTING_NOTES.get(
        provider,
        (PROVIDERS.get(provider, provider), "Token values are read from the provider export when available; missing values are not inferred."),
    )
    if provider == "copilot" and turn_count <= 1:
        description = "Session totals come from shutdown usage. For a single-turn session, those totals can be assigned directly to the turn. Output tokens are also summed from assistant messages."
    common_intro = f"Usage is read from persisted {provider_name} data when available. "
    description = common_intro + description
    return (
        '<section class="token-accounting-note" aria-labelledby="token-accounting-title">'
        '<div class="token-accounting-icon" aria-hidden="true">↔</div>'
        '<div><span class="section-kicker">TOKEN ACCOUNTING</span>'
        f'<h2 id="token-accounting-title">How {esc(provider_name)} usage is calculated</h2>'
        f'<p>{esc(description)}</p></div></section>'
    )
...
def statistics_group_key(session: dict, group: str, provider: str, summary: dict, time_range: str = "all") -> str:
    ...
    if group == "timeline":
        return datetime.fromtimestamp(timestamp).strftime("%H:00") if time_range == "today" else date.isoformat()
    if group == "today":
        return datetime.fromtimestamp(timestamp).strftime("%H:00")
    if group == "month":
...
def render_timeline_svg(points: dict[str, dict[str, float]], time_range: str = "all", provider: str = "copilot") -> str:
...
def render_statistics(summaries: list[tuple[str, dict]], group: str, selected_group: str | None = None, time_range: str = "all", provider: str = "copilot") -> str:
    ...
    cutoff = start_of_today if time_range == "today" or group == "today" else (datetime.now().timestamp() - range_days * 86400 if range_days else None)
    if cutoff is not None and (group in {"timeline", "today"}):
        summaries = [
    ...
    timeline_points: dict[str, dict[str, float]] = (
        {f"{hour:02d}:00": {"tokens": 0.0, "cost": 0.0} for hour in range(24)}
    ...
    rows = "".join(
        f'<tr><td data-sort="{esc(label, quote=True)}"><a class="stats-group-link stats-navigation" href="/?{urlencode({"view": "statistics", "provider": provider, "group": group, "group_value": label})}">{esc(label)}</a></td><td data-sort="{bucket["sessions"]}">{bucket["sessions"]:,}</td>'
        + "".join(f'<td data-sort="{bucket["tokens"][key] or 0}">{fmt(bucket["tokens"][key])}</td>' for key in TOKEN_KEYS)
        + "".join(f'<td data-sort="{bucket["costs"][key]:.6f}">{fmt_cost(bucket["costs"][key])}</td>' for key in TOKEN_KEYS)
        + f'<td data-sort="{sum(bucket["tokens"].values())}">{fmt(sum(bucket["tokens"].values()))}</td><td data-sort="{bucket["cost"]:.6f}">{fmt_cost(bucket["cost"])}</td></tr>'
    ...
    if selected_group in groups:
        if group == "model":
            session_rows = "".join(
                f'<tr><td data-sort="{esc(session.get("id") or "Unavailable", quote=True)}"><a class="stats-group-link stats-navigation" href="/?{urlencode({"view": "operational", "provider": provider, "session": session.get("id", "")})}">{esc(session.get("id") or "Unavailable")}</a></td><td data-sort="{esc(session.get("name") or session.get("id"), quote=True)}">{esc(session.get("name") or session.get("id"))}</td><td data-sort="{esc(PROVIDERS.get(provider, provider), quote=True)}">{esc(PROVIDERS.get(provider, provider))}</td><td data-sort="{esc(model_row["model"], quote=True)}">{esc(model_row["model"])}</td><td data-sort="{sum(value or 0 for value in model_row["tokens"].values())}">{fmt(sum(value or 0 for value in model_row["tokens"].values()))}</td><td data-sort="{sum(model_row["costs"].values()) if model_row["priced"] and model_row["costs"] else 0:.6f}">{fmt_cost(sum(model_row["costs"].values()) if model_row["priced"] else None)}</td></tr>'
                for provider, summary, session, model_row in groups[selected_group]["items"]
            )
        else:
            session_rows = "".join(
                f'<tr><td data-sort="{esc(session.get("id") or "Unavailable", quote=True)}"><a class="stats-group-link stats-navigation" href="/?{urlencode({"view": "operational", "provider": provider, "session": session.get("id", "")})}">{esc(session.get("id") or "Unavailable")}</a></td><td data-sort="{esc(session.get("name") or session.get("id"), quote=True)}">{esc(session.get("name") or session.get("id"))}</td><td data-sort="{esc(PROVIDERS.get(provider, provider), quote=True)}">{esc(PROVIDERS.get(provider, provider))}</td><td data-sort="{esc(session.get("pricingModel") or session.get("model") or "Unavailable", quote=True)}">{esc(session.get("pricingModel") or session.get("model") or "Unavailable")}</td><td data-sort="{sum(value or 0 for value in session.get("tokens", {}).values())}">{fmt(sum(value or 0 for value in session.get("tokens", {}).values()))}</td><td data-sort="{session.get("costUsd") or 0:.6f}">{fmt_cost(session.get("costUsd"))}</td></tr>'
                for provider, summary, session in groups[selected_group]["items"]
            )
        related = f'<section class="stats-related"><div class="section-heading"><div><span class="section-kicker">SELECTED GROUP</span><h2>Sessions in {esc(selected_group)}</h2></div></div><div class="stats-table-wrap"><table class="stats-table related-table"><thead><tr><th data-sort-column="0">Session ID<span class="sort-arrow default" aria-hidden="true"></span></th><th data-sort-column="1">Session name<span class="sort-arrow default" aria-hidden="true"></span></th><th data-sort-column="2">Provider<span class="sort-arrow default" aria-hidden="true"></span></th><th data-sort-column="3">Model<span class="sort-arrow default" aria-hidden="true"></span></th><th data-sort-column="4">Total tokens<span class="sort-arrow default" aria-hidden="true"></span></th><th data-sort-column="5">Total cost<span class="sort-arrow default" aria-hidden="true"></span></th></tr></thead><tbody>{session_rows}</tbody></table></div></section>'
...
def render(root: Path, selected: str | None, selected_turn: int | None = None, selected_metric: str | None = None, provider: str = "copilot", show_empty: bool = False, selected_raw: bool = False, view: str = "sessions", group: str = "today", selected_group: str | None = None, time_range: str = "all", import_error: str | None = None, session_offset: int = 0, sessions_override: list[dict] | None = None, statistics_override: list[tuple[str, dict]] | None = None, scan_bootstrap: str = "", settings_action: str | None = None) -> str:
        ...
        if unlinked_subagents:
            def delegated_rows(agents: list[dict], depth: int = 0) -> str:
                ...
                for agent in agents:
                    ...
                    rows.append(
                        f'<tr><td data-sort="{esc(agent.get("name") or agent.get("id"), quote=True)}">{indent}{esc(agent.get("name") or agent.get("id"))}</td><td data-sort="{total}">{fmt(total)}</td><td data-sort="{agent.get("costUsd") or 0:.6f}">{fmt_cost(agent.get("costUsd"))}</td></tr>'
                    )
                    nested = agent.get("subagents") if isinstance(agent.get("subagents"), list) else []
    ...
    view_tabs += '<script>(function(){function sortRows(table,col,asc){var tbody=table.tBodies[0];var rows=Array.from(tbody.querySelectorAll("tr"));rows.sort(function(a,b){var ca=a.cells[col];var cb=b.cells[col];if(!ca||!cb)return 0;var sa=ca.getAttribute("data-sort");var sb=cb.getAttribute("data-sort");if(sa===null)sa=ca.textContent.trim();if(sb===null)sb=cb.textContent.trim();var na=parseFloat(sa);var nb=parseFloat(sb);if(!isNaN(na)&&!isNaN(nb)&&String(na)===sa.trim()&&String(nb)===sb.trim()){return asc?na-nb:nb-na}return asc?sa.localeCompare(sb):sb.localeCompare(sa)});rows.forEach(function(r){tbody.appendChild(r)})}function setSort(table,col,asc){sortRows(table,col,asc);var cells=table.tHead.rows[0].cells;Array.from(cells).forEach(function(th){th.removeAttribute("data-sort-dir");th.classList.remove("sort-active");var existing=th.querySelector(".sort-arrow.asc, .sort-arrow.desc");if(existing)existing.remove()});var th=cells[col];th.setAttribute("data-sort-dir",asc?"asc":"desc");th.classList.add("sort-active");var arrow=document.createElement("span");arrow.className="sort-arrow "+(asc?"asc":"desc");arrow.setAttribute("aria-hidden","true");th.appendChild(arrow)}function initTable(table,defaultCol){var rows=table.tBodies[0].querySelectorAll("tr");if(rows.length<2)return;setSort(table,defaultCol,false)}document.addEventListener("click",function(event){var th=event.target.closest(".stats-table th");if(!th)return;var col=parseInt(th.getAttribute("data-sort-column"));if(isNaN(col))return;var table=th.closest("table");if(!table||!table.tBodies.length)return;var curDir=th.getAttribute("data-sort-dir");var asc=curDir!=="asc";setSort(table,col,asc)});document.querySelectorAll(".stats-table").forEach(function(t){var th=t.querySelector("th[data-sort-column]");if(th)initTable(t,parseInt(th.getAttribute("data-sort-column")))});})();</script>'
    stats_sidebar = (
...
.turn-invocation{display:grid;grid-template-columns:150px minmax(0,1fr);align-items:start;gap:10px;padding:10px;border:1px solid #263247;border-radius:9px;background:#101722}
.invocation-name{display:grid;width:auto;padding-top:5px;color:#91a2b9;font-size:9px;font-weight:700;letter-spacing:.03em}
.invocation-name>span:first-child{display:flex;align-items:center;gap:7px}
.invocation-model{display:grid;gap:2px;margin-top:8px;padding:7px 8px;border:1px solid #30415a;border-radius:7px;background:#0b121d;box-shadow:inset 3px 0 #4f8cff}
